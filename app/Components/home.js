...
export default function Home({onSongClick,playing,tracks,artists,albums,song_playing_data,isFetching}) {
                    ...
                    <div className='flex flex-row gap-5 p-2 w-full overflow-scroll max-[700px]:gap-3 max-[400px]:gap-2'>
                        {isFetching ? (
                            ...
                            artists.map((artist, index) => (
                                ...
                                <img key={index} alt={`${artist.name}'s Picture`} src={`${artist.picture_xl}`} className=" h-full w-40 mr-3 rounded-full object-cover aspect-square hover:scale-105 ease-in-out duration-300 active:blur-sm active:duration-100"/>
                            ))
                        )}
                    </div>
                </div>
                    ...
                    <div className='flex flex-row gap-5 p-2 overflow-scroll max-[700px]:gap-3 max-[400px]:gap-2'>
                        {isFetching ? (
                            ...
                            albums.map((album, index) => (
                                ...
                                <div className=' overflow-hidden blur-sm fixed'>
                                <img key={index} alt={`${album.name}'s Picture`} src={`${album.cover_xl}`} className=" h-40 w-40 mr-3 rounded-md object-cover aspect-square hover:scale-105 ease-in-out duration-300 blur-md overflow-hidden"/>
                                </div>
                                <img key={index} alt={`${album.name}'s Picture`} src={`${album.cover_xl}`} className=" h-full w-40 mr-3 rounded-md object-cover aspect-square hover:scale-105 z-10 ease-in-out duration-300"/>
                                </div>
                            ))
                        )}
                    </div>
